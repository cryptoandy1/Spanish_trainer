...
- **Never invent Spanish content the user didn't actually produce or see** — for phrases, vocab, translations, and examples. Anything not backed by the source conversation should be `null`/omitted with `needsReview: true`, not filled in from general Spanish knowledge. The whole point of this app is that it mirrors *this user's* material.
